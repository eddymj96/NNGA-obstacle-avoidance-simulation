...
sensor = PerfectObservationSensor(wall, 0.1, pi/8, 1);
% 
% for x=1:length(wall)
%     
...
for outer_loop = 1:(sim_time/dT)
    ...
    %----------------------------------------------%
    % Run Model
    V = sensor.detect(vehicle.xx, vehicle.y, vehicle.psi);
    vehicle.update(V, euler)
...
figure(6); plot(vehicle.x_matrix(20, :),vehicle.x_matrix(19, :)); xlabel('y, m'); ylabel('x, m');

figure(7); plot(vehicle.x_matrix(19, :)); xlabel('x, m');

figure(8);plot(vehicle.x_matrix(24, :)); xlabel('psi, rad'); 

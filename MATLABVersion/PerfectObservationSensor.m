classdef PerfectObservationSensor < handle
    ...
    methods
        ...
        function output = detect(obj,x_current, y_current, heading)
            xpos = x_current;% Assuming the previous referred to this
                                               %"+0.1" was just fixed and not a funciton of input
            ypos = y_current + obj.sensorPosition;
            ...
            for i = 1:length(obj.ObstacleMatrix)
                ...
                x1 = coordinate1(1); y1 =  coordinate1(2);
                x2 = coordinate2(1); y2 =  coordinate2(2);
                ...
                if (r*dr)^2 - D^2 > 0  % Discriminant, postive = intersections, negative = no intersections
                    ...
                    if LB < x_m && x_m < RB
                    	if RB <= 0 
                        	output = [norm([x_m, y_m]), 1];
                    	elseif LB >= 0
                            output = [1,norm([x_m, y_m])];
                        else
                            if x_m <= 0
                                output = [norm([x_m, y_m]), c1];
                            else 
                                output = [c1, norm([x_m, y_m])];
                            end
                        end
                        ...
                        if norm(lefts(IBL, :) - [x_m, y_m]) < norm(rights(IBR, :) - [x_m, y_m])
                            if RB <= 0 
                            	output = [norm(lefts(IBL, :)), 1];
                            elseif LB >= 0
                                output = [1, norm(lefts(IBL, :))];
                            else
                                output = [norm(lefts(IBL, :)), c1];
                            end
                        else
                            if RB <= 0 
                                output = [norm(rights(IBR, :)), 1];
                            elseif LB >= 0
                                output = [1, norm(lefts(IBL, :))];
                            else
                                output = [c1, norm(lefts(IBL, :))];
                            end
                        end
                    end
